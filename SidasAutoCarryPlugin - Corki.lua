...
function PluginOnTick()
	...
	if Target and (AutoCarry.MainMenu.AutoCarry or AutoCarry.MainMenu.MixedMode) then
		...
		if RREADY and Menu.useR then Cast(SkillR, Target) end
	end
	--> Missile KS
...
function castQ(target)
	...
	if qPred and GetDistance(qPred) <= qRange then
		...
		elseif GetDistance(qPred) <= 600 then
...
function missileKS()
	for i, enemy in ipairs(GetEnemyHeroes()) do
		...
		if enemy and not enemy.dead and enemy.health < rDmg then Cast(SkillR, enemy) end
	end
end
