...
function Menu()
	...
	AutoCarry.PluginMenu:addParam("AvoidWallsWithQ", "Avoid Hitting Walls with Q (Beta)", SCRIPT_PARAM_ONOFF, false)
	AutoCarry.PluginMenu:addParam("SatchelJumpWithE", "Satchel Jump with E", SCRIPT_PARAM_ONOFF, true)
	AutoCarry.PluginMenu:addParam("HarassWithE", "Harass with E", SCRIPT_PARAM_ONOFF, false)
	AutoCarry.PluginMenu:addParam("KillstealMinHitchance", "Killsteal Min. Req. Hitchance", SCRIPT_PARAM_SLICE, 60, 0, 90, 0)
	AutoCarry.PluginMenu:addParam("AFKKillstealDisable", "AFK Killsteal Disable", SCRIPT_PARAM_ONOFF, true)
	AutoCarry.PluginMenu:addParam("AFKKillstealDisableSeconds", "AFK Killsteal Disable Seconds", SCRIPT_PARAM_SLICE, 120, 10, 600, 0)
end
...
function IsSummonerAFK()
	...
	if not myHeroLastPos or not tick or not afkTick or myHero.x ~= myHeroLastPos.x or myHero.z ~= myHeroLastPos.z then
		...
		return false
	elseif tick > (afkTick + (AutoCarry.PluginMenu.AFKKillstealDisableSeconds * 1000)) and myHero.x == myHeroLastPos.x and myHero.z == myHeroLastPos.z then
...
function PluginOnTick()
	...
	if AutoCarry.PluginMenu.Killsteal and IsTickReady(50) and not lastAFKStatus then
		KillSteal()
	end
	
	if (AutoCarry.MainMenu.LaneClear or AutoCarry.MainMenu.MixedMode) and IsTickReady(50) and AutoCarry.PluginMenu.SmartFarmWithQ and not IsMyManaLow() then
		SmartFarmWithQ()
	end
end
...
function IsTickReady(tickFrequency)
	-- Improves FPS
	if tick ~= nil and math.fmod(tick, tickFrequency) then
		return true
	else
		return false
	end
...
function Harass()
	CastQ()
	if AutoCarry.PluginMenu.HarassWithE then CastE() end
end
...
function IsQWallCheckOK(position)
	if position and not AutoCarry.PluginMenu.AvoidWallsWithQ or (AutoCarry.PluginMenu.AvoidWallsWithQ and not IsWall(D3DXVECTOR3(position.x, myHero.y, position.z))) then
		return true
	else
		return false
	end
...
function CastWActivate()
	if satchelChargeExists and pendingSatchelChargeActivation ~= nil then
		if pendingSatchelChargeActivation == "satcheljump" then
			...
			if AutoCarry.PluginMenu.SatchelJumpWithE then CastE() end
			pendingSatchelChargeActivation = nil
		end
	end
end
	...
	for _, enemy in pairs(AutoCarry.EnemyTable) do
		if ValidTarget(enemy, RRange) and not enemy.dead then
			if (enemy.health + 30) < getDmg("R", enemy, myHero) and IsValidHitChanceCustom(SkillR, enemy) then
				...
				if RReady and enemyPos and GetDistance(enemyPos) < RRange then
...
function getScalingHitChanceFromDistance(SkillRange, Target)
	local minHitChance = AutoCarry.PluginMenu.KillstealMinHitchance
